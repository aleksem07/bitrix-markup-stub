...
var addSlider = function addSlider(sliderId, minId, maxId, options) {
  document.addEventListener('DOMContentLoaded', function () {
    var slider = document.getElementById(sliderId);
    var inputMin = minId ? document.getElementById(minId) : null;
    var inputMax = document.getElementById(maxId);
...
// =require add-slider.js
addSlider('cbd', null, 'cbd-value-max', {
  start: [65],
  connect: 'lower',
  tooltips: {
    to: function to(value) {
...
document.addEventListener('DOMContentLoaded', function () {
  var addCheckedFilters = function addCheckedFilters() {
    var checkboxes = document.querySelectorAll('.checkbox_input');
    var checkedListBox = document.querySelector('.checked_list');
    checkboxes.forEach(function (checkbox, idx) {
      checkbox.addEventListener('change', function () {
        var itemValue = checkbox.value.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        if (checkbox.checked) {
          createLi(checkbox, idx, itemValue, checkedListBox);
        } else {
          var listItem = checkedListBox.querySelector("li[data-value=\"".concat(idx, "\"]"));
          if (listItem) {
            listItem.remove();
          }
        }
...
var createLi = function createLi(checkbox, idx, value, checkedElements) {
...
var initializeChecked = function initializeChecked() {
  var checkboxes = document.querySelectorAll('.checkbox_input');
  var checkedListBox = document.querySelector('.checked_list');
  checkboxes.forEach(function (checkbox, idx) {
    var itemValue = checkbox.value.replace(/</g, '&lt;').replace(/>/g, '&gt;');
